...
import {
  ...
  SiTailwindcss, SiBootstrap, SiGit, SiGithub, SiFlask, SiVercel, SiSupabase, SiOpenai
} from 'react-icons/si';
...
export default function SplashScreen({ duration = 5000, once = 'session' }) {
  ...
  const markSeen = useCallback(() => {
    try {
      ...
      else if (once === 'local') localStorage.setItem('introSeen', '1');
    } catch {}
  }, [once]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const ids = { raf: 0, t1: 0, t2: 0 };

    ids.raf = requestAnimationFrame(() => {
      if (cancelled) return;
      setShow(true);

      ids.t1 = window.setTimeout(() => {
        if (!cancelled) setFade(true);
      }, Math.max(0, duration - 600));

      ids.t2 = window.setTimeout(() => {
        if (cancelled) return;
        setShow(false);
        setFade(false);
        markSeen();
    ...
    return () => {
      cancelled = true;
      if (ids.raf) cancelAnimationFrame(ids.raf);
      if (ids.t1) clearTimeout(ids.t1);
      if (ids.t2) clearTimeout(ids.t2);
    };
  }, [duration, once, reduceMotion, markSeen]);
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-2xl px-6">
        ...
        <div className="mt-6 flex justify-center">
          <button onClick={onSkip} className="rounded-lg border border-zinc-700 px-3 py-1.5 text-sm text-zinc-200 hover:bg-zinc-800/60" >
            Skip intro
          </button>
        </div>
      </div>
